...
// }
/*###############################################*/
void BLVD20KM_hieplm::serialInit(int serial_port, uint8_t address)
{
	...
	if(serial_port<0)
	{
		...
		exit(-1) ;
	}else ROS_INFO("Device 0x%x is connected ",address);
}

/*###############################################*/
BLVD20KM_hieplm::BLVD20KM_hieplm(int serial_port ,uint8_t address)
{
	this->serial_port = serial_port;
	this->address = address;
	ROS_INFO("Device 0x%x is connecting port(%d)", address, serial_port);
	serialInit(serial_port,address);
}
